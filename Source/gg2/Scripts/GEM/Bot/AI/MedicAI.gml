...
with Character
{
    ...
    and !cloak
    and (player.class != CLASS_MEDIC or hp < 40 or other.object.currentWeapon.ubering)
    ...
    and healer == -1//target is not healed, dont steal patient, it's not effective
    {
        ...
        {
            if ((hp/maxHp)*1.1 < (other.patient.hp/other.patient.maxHp)//i am more hurt (*1.5 to prevent switching target every step) | Changed the constant because medics were too sticky.
...
if patient != -1 and isHealing
{
    ...
    {
        if task != 'objective' and object.intel == false
        and !abs(abs(object.x-patient.x)-100)<32
            {
            if object.x > (patient.x-patient.image_xscale*100)
            dir=-1;
            if object.x < (patient.x-patient.image_xscale*100)
            dir=1;
            }
...
if !object.currentWeapon.ubering and object.currentWeapon.uberReady and !object.ubered and nearestEnemy != -1
{
