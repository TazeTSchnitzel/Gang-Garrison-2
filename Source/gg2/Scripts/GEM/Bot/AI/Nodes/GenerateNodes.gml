...
for(i=0; i<size; i+=1)// Get rid of all those that are in a horizontal row, save the edges.
{
    if !inRow// The beginning of a row
    {
        ...
        ds_list_add(newNode.network, newNode)
        inRow = 1;
    }
    else if i < size-1
    {
        if positions[i, 0]+1 != positions[i+1, 0] or positions[i, 1] != positions[i+1, 1]// If this is the last node
        {
            ...
            // Make a connection between this node and the last one
            node = ds_list_find_value(nodeList, ds_list_size(nodeList)-2)
            ds_list_add(newNode.connections, node)
            ds_list_add(newNode.distance, point_distance(newNode.x, newNode.y, node.x, node.y))
            ds_list_add(newNode.commands, sign(newNode.x-node.x))
            ds_list_add(newNode.network, node)
            ds_list_add(node.connections, newNode)
            ds_list_add(node.distance, point_distance(newNode.x, newNode.y, node.x, node.y))
            ds_list_add(node.commands, sign(node.x-newNode.x))
            ds_list_add(node.network, newNode)
